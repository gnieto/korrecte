...
impl KubeObjectLoader {
    pub fn read(input: &str) -> LoadResult {
        ...
        let decoded_yaml =
            YamlLoader::load_from_str(input).map_err(|_| KorrecteError::FailedToLoadYamlFile)?;
    ...
    pub fn read_file(path: &Path) -> LoadResult {
        ...
        let file_content =
            fs::read_to_string(path).map_err(|_| KorrecteError::FailedToLoadYamlFile)?;
    ...
    fn yaml_object_to_kube_object_type(yaml: &Yaml) -> Result<KubeObjectType, KorrecteError> {
        ...
        let (api_version, kind) = {
            ...
            emitter
                .dump(&yaml)
                .map_err(|_| KorrecteError::UnrecognizedObject)?;
