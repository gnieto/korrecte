...
use crate::config::in_cluster::InClusterConfig;
...
pub fn load_config() -> Result<Box<dyn ClusterConfig>> {
    ...
    if let Ok(in_cluster) = InClusterConfig::new() {
        return Ok(Box::new(in_cluster));
    }

    Err(anyhow!("Could not load kube configuration"))
}
...
impl Config {
    pub fn resolve(&self) -> Result<RemoteCluster, ResolutionError> {
        ...
        let current_config = RemoteCluster::new(cluster.clone(), context.clone(), auth.clone());
        Ok(current_config)
    }
